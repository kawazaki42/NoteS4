...
== через действительную часть комплексной мощности

комплексная мощность вычисляется:
$ underline(S) = underline(U) dot uI^* = P + j Q $

$ P = "Re"(underline(S)) = "Re"(underline(U) dot uI^*) $

комплекс тока двухполюсника равен $uI_1$

комплекс напряжения равен $euab$

#uis.at(0)

$ P
= "Re"((uab) dot (complex(#(-0.2771), #0.9293)))
= "Re"(complex(#(-31.036), #13.832))
= -31.036 "Вт"
$

/*

(22.814+26.594j) * (−0.2771+0.9293j)

(-31.035563600000003+13.8318528j)

*/

== через коэффицент мощности

$ cos phi = P/S => P = S cos phi = U I cos phi $

$U, I$ -- среднеквадратические значения

$phi$ -- угол сдвига фаз между напряжением и током

// 35.038827491798294

$ U_(a b) = abs(euab) = abs(uab) = 35.039 space В $

// 0.9697334169760264

$ I_1 = abs(uI_1) = abs(uis.at(#0)) = 0.970 space А $

$ phi = psi_(U a b) - psi_(I 1)
  = 49.375 degree -(-106.694 degree)
  = #rnd(49.375 + 106.694) degree $

#let p_from_cos = 35.039 * 0.970 * calc.cos(156.069deg)

$ P &= U_(a b) I_1 cos phi = \
    &= 35.039 dot 0.970 dot cos 156.069 degree = \
    &= #rnd(p_from_cos) "Вт" $
...
- $I_1 = abs(uis.at(#0)) = 0.970 space А$
- $I_(1 m) = sqrt(2) I_1 = sqrt(2) dot 0.970 = #rnd(calc.sqrt(2) * 0.970) space А$
- $psi_(i_1) = -106.604 degree = #rnd(-106.604deg.rad()) "рад"$

диапазон углов: $omega t in [0; 2 pi]$

$ i_1
= 1.372 sin(omega t - 106.604 degree)
= 1.372 sin(omega t - 1.861 "рад")
space А
$
