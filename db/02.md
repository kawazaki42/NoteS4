...
предметная область: лабораторный анализ содержаний элементов в скважине
...
- геологический отдел
  ...
  - заполнение геологической таблицы
- лаборатория
  - получение наряд-заказа
  - анализ проб
  - контроль качества
  - создание протокола
...
1. Лаборатория создает наряд-заказ с атрибутами:
   - наименование объекта (удоканское месторождение)
   - номер скважины
   - дата создания
   - ответственное лицо
2. каждый наряд заказ содержит информацию о множестве проб:
   - код пробы
   - глубина взятия пробы
3. геологический отдел для каждого наряд заказа дает информацию об
   элементах и лабораторных методах, которые будут исследованы
   для каждой пробы наряд заказа
   - для элементов:
     - наименование
     - обозначение (Au, Ag, ...)
   - для метода:
      - наименование
      - рейтинг точности
4. ...содержаний лаборатория выдает протокол для каждого наряд заказа
   - номер
   - дата
   - ответственное лицо
   - статус
   ...
   - элемент
   - значение
   - содержание элемента по данному методу в пробе
   - номер заказа?
5. после проведения контроля качества геологическим отделом
   может потребоваться проведение доп. анализов и соот-но
   создание нового протокола по наряд заказу
